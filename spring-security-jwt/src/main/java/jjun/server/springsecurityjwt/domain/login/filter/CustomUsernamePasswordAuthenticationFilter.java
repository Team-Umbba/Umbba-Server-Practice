package jjun.server.springsecurityjwt.domain.login.filter;
...
public class CustomUsernamePasswordAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
    ...
    private static final String DEFAULT_LOGIN_REQUEST_URL = "/login";   // "/login"으로 오는 요청을 처리한다.
    private static final String HTTP_METHOD = "POST";   // 로그인 API의 HTTP Method는 POST
    ...
    // "/login" path에 대한 접근과 POST 요청에 대해 매칭된다.
    private static final AntPathRequestMatcher DEFAULT_LOGIN_PATH_AND_POST_MATCHER = new AntPathRequestMatcher(DEFAULT_LOGIN_REQUEST_URL, HTTP_METHOD);

    private final ObjectMapper objectMapper;

    public CustomUsernamePasswordAuthenticationFilter(ObjectMapper objectMapper) {
        super(DEFAULT_LOGIN_PATH_AND_POST_MATCHER);   // 위에서 지정한 "login"+POST 요청을 처리하기 위해 부모클래스의 생성자로 설정해준다.
